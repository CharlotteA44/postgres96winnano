...
#### Install PostGis Packaging
RUN $URL3 = $('http://download.osgeo.org/postgis/windows/pg12/postgis-bundle-pg12-3.0.2x64.zip') ; \
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 ; \
    Invoke-WebRequest -Uri $URL3 -OutFile 'C:\\postgis.zip' ; \
    Expand-Archive 'C:\\postgis.zip' -DestinationPath 'C:\\' ; \
	Copy-item -Force -Recurse -Verbose 'C:\\postgis-bundle-pg12-3.0.2x64\\*' -Destination 'C:\\pgsql' ; \
	Remove-Item -Path 'C:\\postgis.zip' ; \
	Remove-Item -Recurse -Force –Path 'C:\\postgis-bundle-pg12-3.0.2x64'
